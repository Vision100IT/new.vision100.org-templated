...
const webformUUID = "03a0ad81-037c-48d1-a78d-6bfd12ebbfbd";
	...
	handleSubmit(e) {
		...
		if (errorMessage !== "") {
			...
			form.append(
				...
				escape(this.state.name).replace(
					/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2694-\u2697]|\uD83E[\uDD10-\uDD5D])/g,
					""
				)
			);
			...
			form.append(
				...
				escape(this.state.attendedBefore).replace(
					/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2694-\u2697]|\uD83E[\uDD10-\uDD5D])/g,
					""
				)
			);
			form.append(
				...
				escape(this.state.registrationType).replace(
					/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2694-\u2697]|\uD83E[\uDD10-\uDD5D])/g,
					""
				)
			);
			form.append(
				...
				escape(this.state.participatingFrom).replace(
					/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2694-\u2697]|\uD83E[\uDD10-\uDD5D])/g,
					""
				)
			);
			...
			form.append(
				...
				escape(this.state.paymentMethod).replace(
					/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2694-\u2697]|\uD83E[\uDD10-\uDD5D])/g,
					""
				)
			);

			var totalAmount = this.state.registrationType === "full" ? 50 : 50;
			...
			registrationForm = (
				<section>
					<form onSubmit={this.handleSubmit}>
						...
						<label>
							<strong>Attended Preachers Workshop before?</strong> {requiredField}
						</label>
						<br />
						<br />
						<label>
						...
						<label>
							<strong>Which location will you participate from?</strong> {requiredField}
						</label>
						<br />
						<br />
						<select
							name="participatingFrom"
							value={this.state.participatingFrom}
							onChange={this.handleChange.bind(this)}
						>
						...
						<label>
							<strong>Registration Cost?</strong> {requiredField}
						</label>
						<br />
						<br />
						<label>
							<input
								...
								value="full"
								onChange={this.handleChange.bind(this)}
								checked={this.state.registrationType === "full"}
							/>{" "}
							$50&nbsp;
						</label>
						<br />
						{/* <label>
							<input
								...
								value="workshopOnly"
								onChange={this.handleChange.bind(this)}
								checked={this.state.registrationType === "workshopOnly"}
							/>{" "}
							$10 if you already have a copy of Setting Hearts on Fire &nbsp;
						</label> */}
						<br />
						<br />
			...
			formSubmitted = (
				<div>
					{this.state.submissionID && this.state.paymentMethod === "paypal" ? (
						<GenericPaypal sid={this.state.submissionID} totalCost={this.state.totalAmount} eventName="PW" />
					) : (
						<>
							...
							<p>
								...
								<br />
								Reference: PW {this.state.submissionID}
								<br />
							</p>
							<p>
